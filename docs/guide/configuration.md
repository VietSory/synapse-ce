...
| `SYNAPSE_REACH_GOBIN` | `true` | RAISE-ONLY Go-binary affected-symbol reachability (Tier-2). A compiled Go binary in the workspace whose `.gopclntab` function table contains a matched vulnerable function raises that finding's urgency. It NEVER concludes `not_reachable`: absence is no coverage, because a binary lacking a pclntab (non-Go), an inlined or dead-code-eliminated function, or a symbol resolved through a function pointer is invisible. So it can only raise, never suppress, and its verdict never becomes an OpenVEX `not_affected`. Reads binaries only, builds nothing. Needs judgments. |
