...
func TestCIContextFromEnvGitLabAndBitbucket(t *testing.T) {
	tests := []struct {
		name string
		env  map[string]string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got.Provider != tt.want.Provider || got.Branch != tt.want.Branch || got.RunID != tt.want.RunID || got.PullRequest != tt.want.PullRequest || got.TargetBranch != tt.want.TargetBranch || got.RepoSlug != tt.want.RepoSlug || got.HeadSHA != tt.want.HeadSHA {
