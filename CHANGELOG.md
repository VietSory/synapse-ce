...
- **Owned secret scanning now ships 123 credential detectors.** The default catalog grows from 78 to 123 provider/config-specific rules, adding coverage for Okta, Auth0, Cloudflare, Firebase, Discord bots, Heroku, Mapbox, Netlify, PagerDuty, Snyk, Azure/Entra, AWS session credentials, GCP OAuth, kubeconfig, HTTP Authorization headers, npm/PyPI/netrc credentials, CI/CD and observability tokens, and additional SaaS/API keys. Every new detector retains the scanner's keyword prefilter, redaction, placeholder allow-list behavior, and catalog parity contract, with positive and placeholder-negative fixtures. This advances EPIC #860 D6.2 without changing the scanner's read-only/offline posture.
