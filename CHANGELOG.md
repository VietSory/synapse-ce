...
- **Opt-in active secret verification.** `SYNAPSE_SECRET_VERIFY_ENABLED=false` keeps ordinary scanning fully offline. When explicitly enabled, detected workspace GitHub, AWS and Vault credentials receive only provider-minimal read-only checks; raw values never enter DTOs, logs, persisted scan results, or evidence. Rejected and unknown credentials remain findings and are routed to verification; history and image-rootfs secrets are never egressed.
