...
// CredentialLocator is the entire global pre-authentication result, never an authenticated
// principal. Intentionally absent: person, email, subject, role, connection and arbitrary
// metadata. The caller MUST bind OrganizationID, read the authoritative credential/membership
// through an RLS-scoped port, and reject a disabled credential before authenticating.
