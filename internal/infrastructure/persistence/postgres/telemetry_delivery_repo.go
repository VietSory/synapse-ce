...
func (r *TelemetryRepository) IngestDelivery(ctx context.Context, batch ports.TelemetryDeliveryBatch) (ports.TelemetryDeliveryResult, error) {
	...
	err = WithContextTenant(ctx, r.pool, func(tx pgx.Tx) error {
		...
		if m.Epoch > currentEpoch {
			if _, err := tx.Exec(ctx, `UPDATE telemetry_delivery_streams SET current_epoch=$1, updated_at=$2 WHERE tenant_id=$3 AND stream_id=$4`,
		...
		if _, err := tx.Exec(ctx, `UPDATE telemetry_delivery_batches
			SET state=CASE WHEN sequence <= $1 THEN 'acknowledged' ELSE 'telemetry_durable' END, updated_at=$2
		...
		gaps, err := queryTelemetryGapsTx(ctx, tx, telemetryGapQuery{tenantID: batch.TenantID, hostID: batch.HostID, assetID: batch.AssetID, streamID: m.StreamID, epoch: m.Epoch})
		if err != nil {
			return err
		}
		result = ports.TelemetryDeliveryResult{ACK: ports.TelemetryDeliveryACK{Priority: m.Priority, Epoch: m.Epoch, Through: ack}, NewEvents: newEvents, Gaps: gaps}
		return nil
	})
	if err != nil {
		return ports.TelemetryDeliveryResult{}, err
...
func preflightTelemetryDeliverySequences(ctx context.Context, tx pgx.Tx, batch ports.TelemetryDeliveryBatch) (bool, error) {
	...
	for i := range batch.Envelopes {
		...
		err = tx.QueryRow(ctx, `SELECT event_id,event_digest,delivery_key FROM telemetry_delivery_sequences
...
func insertOrValidateTelemetryBatch(ctx context.Context, tx pgx.Tx, batch ports.TelemetryDeliveryBatch) error {
	...
	ct, err := tx.Exec(ctx, `INSERT INTO telemetry_delivery_batches
		...
		batch.TenantID.String(), m.BatchID.String(), batch.AgentID.String(), batch.HostID.String(), batch.AssetID.String(), string(batch.AgentSessionID),
		m.StreamID.String(), int(m.Priority), int64(m.Epoch), int64(m.Sequence), int64(m.PreviousSequence), m.SchemaVersion, batch.KeyID,
		m.PayloadDigest, m.EventTimeMin.UTC(), m.EventTimeMax.UTC(), batch.ReceivedAt.UTC())
	if err != nil {
	...
	err = tx.QueryRow(ctx, `SELECT agent_id,host_id,asset_id,agent_session_id,stream_id,priority,epoch,sequence,previous_sequence,
		schema_version,key_id,payload_digest,event_time_min,event_time_max
		FROM telemetry_delivery_batches WHERE tenant_id=$1 AND batch_id=$2`, batch.TenantID.String(), m.BatchID.String()).Scan(
		&agentID, &hostID, &assetID, &session, &streamID, &priority, &epoch, &sequence, &previous, &schemaVersion, &keyID, &digest, &eventTimeMin, &eventTimeMax)
	if err != nil {
		return fmt.Errorf("read telemetry delivery batch collision: %w", err)
	}
	if agentID != batch.AgentID.String() || hostID != batch.HostID.String() || assetID != batch.AssetID.String() || session != string(batch.AgentSessionID) ||
		streamID != m.StreamID.String() || priority != int(m.Priority) || epoch != int64(m.Epoch) || sequence != int64(m.Sequence) || previous != int64(m.PreviousSequence) ||
		schemaVersion != m.SchemaVersion || keyID != batch.KeyID || digest != m.PayloadDigest || !eventTimeMin.Equal(m.EventTimeMin.UTC()) || !eventTimeMax.Equal(m.EventTimeMax.UTC()) {
...
func insertTelemetryDeliveryEvents(ctx context.Context, tx pgx.Tx, batch ports.TelemetryDeliveryBatch) (int, error) {
	...
	for i := range batch.Envelopes {
		...
		eventCT, err := tx.Exec(ctx, `INSERT INTO telemetry_events
			...
			int64(m.Epoch), m.BatchID.String(), m.EventIDs[i].String(), canonicalJSON, key, batch.ReceivedAt.UTC())
		if err != nil {
			return 0, fmt.Errorf("insert A3 telemetry event: %w", err)
		}
		if eventCT.RowsAffected() != 1 {
			return 0, fmt.Errorf("%w: durable sequence was inserted but telemetry event delivery key already exists", shared.ErrConflict)
		}
		inserted++
	}
	return inserted, nil
...
func telemetryDeliveryACKAndGaps(ctx context.Context, tx pgx.Tx, tenantID, streamID shared.ID, epoch uint64) (uint64, []fleetagent.SeqRange, error) {
	rows, err := tx.Query(ctx, `SELECT sequence FROM telemetry_delivery_sequences WHERE tenant_id=$1 AND stream_id=$2 AND epoch=$3 ORDER BY sequence`,
		tenantID.String(), streamID.String(), int64(epoch))
	if err != nil {
	...
	var ranges []fleetagent.SeqRange
	var previous uint64
	...
	if ranges[0].From > 1 {
		return ranges[0].From - 1, ranges, nil
	}
	return 0, ranges, nil
}
...
func queryTelemetryGapsTx(ctx context.Context, tx pgx.Tx, q telemetryGapQuery) ([]ports.TelemetryGap, error) {
	rows, err := tx.Query(ctx, `SELECT tenant_id,host_id,asset_id,agent_id,agent_session_id,stream_id,priority,epoch,
		...
		FROM telemetry_gaps
		WHERE tenant_id=$1 AND ($2='' OR host_id=$2) AND ($3='' OR asset_id=$3) AND ($4='' OR stream_id=$4)
		  AND ($5::smallint IS NULL OR priority=$5) AND ($6::bigint=0 OR epoch=$6)
		  AND ($7::timestamptz IS NULL OR to_at >= $7) AND ($8::timestamptz IS NULL OR from_at <= $8)
		  AND resolved_at IS NULL ORDER BY epoch,from_sequence`,
		q.tenantID.String(), q.hostID.String(), q.assetID.String(), q.streamID.String(), nullableTelemetryPriority(q.priority), int64(q.epoch),
	...
	for rows.Next() {
		var g ports.TelemetryGap
		var tenant, host, asset, agent, session, stream string
		var priority int
		var epoch, from, to int64
		if err := rows.Scan(&tenant, &host, &asset, &agent, &session, &stream, &priority, &epoch, &from, &to, &g.FromAt, &g.ToAt, &g.DetectedAt, &g.ResolvedAt); err != nil {
			return nil, err
		}
...
func nullableTelemetryPriority(priority *fleetagent.DeliveryPriority) any {
	if priority == nil {
		return nil
	}
...
func nullableTelemetryGapTime(value time.Time) any {
	if value.IsZero() {
		return nil
	}
	return value.UTC()
...
func (r *TelemetryRepository) QueryDeliveryGaps(ctx context.Context, q ports.HuntQuery) ([]ports.TelemetryGap, error) {
	...
	err := WithContextTenant(ctx, r.pool, func(tx pgx.Tx) error {
		var err error
		out, err = queryTelemetryGapsTx(ctx, tx, telemetryGapQuery{tenantID: tenant, hostID: q.HostID, assetID: q.AssetID, priority: priority, since: q.Since, until: q.Until})
		return err
	})
	return out, err
}
...
func (r *TelemetryRepository) BindTelemetryAsset(ctx context.Context, binding ports.TelemetryAssetBinding) error {
	...
	return WithContextTenant(ctx, r.pool, func(tx pgx.Tx) error {
		ct, err := tx.Exec(ctx, `INSERT INTO telemetry_asset_bindings (tenant_id,agent_id,asset_id,updated_at)
			VALUES ($1,$2,$3,$4) ON CONFLICT (tenant_id,agent_id) DO UPDATE
