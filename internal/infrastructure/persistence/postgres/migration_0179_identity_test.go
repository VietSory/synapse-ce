...
func TestMigration0179IdentityRLSInventory(t *testing.T) {
...
func TestCredentialLocatorResolverPortIsExactOnly(t *testing.T) {
	...
	locator := reflect.TypeOf(orgidentity.CredentialLocator{})
	wantFields := []string{"Digest", "OrganizationID", "Kind", "CredentialID"}
	if locator.NumField() != len(wantFields) {
		t.Fatalf("credential locator has %d fields; want only an exact pre-authentication route", locator.NumField())
	}
	for i, want := range wantFields {
		if got := locator.Field(i).Name; got != want {
			t.Fatalf("credential locator field %d = %s, want %s", i, got, want)
		}
	}
}

// This compile/static surface guard keeps the global person_organization_index platform-only.
// Platform workflows may fan a mutation out from one already-known person, but no tenant-facing
// caller may enumerate, discover, or filter memberships through this non-RLS index.
func TestPlatformPersonIndexPortIsExactPersonOnly(t *testing.T) {
	typ := reflect.TypeOf((*ports.PlatformPersonIndex)(nil)).Elem()
	if typ.NumMethod() != 1 {
		t.Fatalf("platform person index has %d methods; want one exact-person fan-out lookup only", typ.NumMethod())
	}
	method := typ.Method(0)
	if method.Name != "OrganizationsForExactPerson" {
		t.Fatalf("platform person index method = %s, want OrganizationsForExactPerson", method.Name)
	}
}
